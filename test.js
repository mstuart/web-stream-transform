...
test("mapStream works with async functions", async (t) => {
  const result = await collectStream(
    createReadable(["a", "b"]).pipeThrough(
      mapStream(async (x) => x.toUpperCase())
    )
  );
...
test("filterStream keeps matching chunks", async (t) => {
  const result = await collectStream(
    createReadable([1, 2, 3, 4, 5]).pipeThrough(
...
test("tapStream passes chunks through unchanged", async (t) => {
  const result = await collectStream(
    createReadable([1, 2, 3]).pipeThrough(tapStream(() => undefined))
  );
  t.deepEqual(result, [1, 2, 3]);
});
...
test("tapStream calls function for side effects", async (t) => {
  const seen = [];
  const result = await collectStream(
    createReadable([1, 2, 3]).pipeThrough(
      tapStream((x) => {
        seen.push(x);
      })
    )
